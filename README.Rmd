...
`marr` measures the reproducibility 
of features per sample pair and sample pairs per feature in high-dimensional
biological replicate experiments. 

### Method

The `marr` paper published in _Journal of American statistical Association_:

> Philtron, Daisy, et al. "Maximum Rank Reproducibility: A Nonparametric Approach to
Assessing Reproducibility in Replicate Experiments." Journal of the American
Statistical Association 113.523 (2018): 1028-1039.
[https://doi.org/10.1080/01621459.2017.1397521](https://doi.org/10.1080/01621459.2017.1397521)
...
The R-package **marr** can be installed from GitHub using the R 
...
The main function in the **marr** package is `marr()`.  The `marr()` function needs one required object
and three optional objects: 
(1) object: a data frame or a matrix or a Summarized Experiment with one assay object with observations
(e.g., metabolites or genes) on the rows and samples as the columns 
(e.g. let's call it `dataSE`). (2) pSamplepairs (Optional) a threshold value that lies
...
Default is 0.75. (3) pFeatures (Optional) a threshold value that lies between
...
Default is 0.75. (4) alpha (Optional) level of significance to control
the False Discovery Rate (FDR). Default is 0.05.
...
The percent reproducible sample pairs per feature can be directly plotted
using the `marrPlotFeatures()` function. 
```
marrPlotFeatures(marrOutput) 
```

The percent reproducible features per sample pair can be directly plotted
using the `marrPlotSamplepairs()` function. 
```
marrPlotFeatures(marrOutput) 
```
...
# Bug reports
Report bugs as issues on the [GitHub repository new issue](https://github.com/Ghoshlab/marr/issues/new)
